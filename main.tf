...
#   vpc_id = data.aws_vpc.dev_vpc_id.id
#   tags = {
#     Tier = "Private"
#   }
# }

data "aws_subnet" "sub_id"{
  vpc_id = data.aws_vpc.dev_vpc_id.id
  # filter {
  #   name = "vpc_id"
  #   values = [data.aws_vpc.dev_vpc_id.id]
  # }
...
output "outputs1" {
  value = data.aws_subnet.sub_id.id
}

output "maintableid" {
  value = data.aws_vpc.dev_vpc_id.main_route_table_id
}
